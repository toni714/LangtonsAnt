...
public class SwingDrawingManager implements DrawingManager {
	...
	public void paint(Graphics g) {
		...
		drawField(g);
		/*
		 * for(Ant ant:GC.ants){ drawAnt(g, ant); }
		 */
	}
		...
		if (GC.initDone) {
			...
			for (int x = 0; x < GC.field.colors.length; x++) {
				for (int y = 0; y < GC.field.colors[x].length; y++) {
					...
					float percent = 1-(GC.field.colors[x][y] / (float) GC.rules.length);
	...
	public void drawAnt(Graphics g, Ant ant) {
		g.drawRect(GC.field.pos.x + (ant.pos.x * GC.antSize), GC.field.pos.y + (ant.pos.y * GC.antSize), GC.antSize,
				GC.antSize);
	}
